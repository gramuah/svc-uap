...
def run_evaluation(ground_truth_file, json_results_file, max_avg_n_proposals=100,
                   tiou_thresholds=np.linspace(0.5, 0.95, 10), subset='validation'):

    anet_proposal = ANETproposal(ground_truth_file, json_results_file, tiou_thresholds=tiou_thresholds,
                                 max_avg_nr_proposals=max_avg_n_proposals, subset=subset, verbose=True, check_status=False)
...
def plot_metric(average_n_proposals, average_recall, recall, tiou_thresholds=np.linspace(0.5, 0.95, 10),
    ...
    colors = ['#e41a1c', '#377eb8', '#4daf4a', '#984ea3', '#ff7f00', '#ffff33', '#a65628', '#f781bf', '#999999', '#737373']
    ...
    ax.plot(average_n_proposals, average_recall, color=colors[0],
            label="tiou = 0.5:0.05:0.95," + " area=" + str(int(np.trapz(average_recall, average_n_proposals) * 100) / 100.),
            linewidth=4, linestyle='-', marker=None)
